...
<head>
    ...
    <title>8068 CAR SQUARE | Enterprise POS System v7.0</title>
    ...
    <style>
        * { font-family: 'Inter', sans-serif; }
        ...
        .status-badge { padding: 2px 10px; border-radius: 12px; font-size: 10px; font-weight: bold; }
...
<script>
    ...
    let appData = {
        ...
        currentUser: null,
        nextIds: { transaction: 100, customer: 50, expense: 20, service: 50, waiting: 1, inventory: 10, ar: 30, ap: 30, jobOrder: 40 },
    ...
    const saved = localStorage.getItem("8068_enterprise_v7");
    if(saved) {
        try {
            ...
            if (!appData.accountsReceivable) appData.accountsReceivable = [];
            if (!appData.accountsPayable) appData.accountsPayable = [];
            if (!appData.jobOrders) appData.jobOrders = [];
    ...
    function saveToLocal() {
        localStorage.setItem("8068_enterprise_v7", JSON.stringify(appData));
        ...
        return `<div class="min-h-screen flex items-center justify-center">
            <div class="card-premium p-8 max-w-md w-full">
                <div class="text-center mb-6">
                    ...
                    <p class="text-cyan-400 text-xs mt-2">Enterprise Management System v7.0</p>
                </div>
                <div class="space-y-4">
                    ...
                    <div class="text-xs text-gray-500 text-center mt-4">
                        <p>Demo Accounts:</p>
                        <p>Owner: owner/owner123 | Manager: manager/manager123</p>
    ...
    // ======================== EXCEL EXPORT WITH WRAPPED TEXT ========================
    function exportToExcel(reportType) {
        ...
        // Auto column widths with proper sizing
        const colWidths = [];
        if (data.length > 0) {
    ...
    function renderDashboard() {
        ...
        const waitingCount = appData.waitingList.filter(w => w.status !== 'completed').length;
        const arTotal = appData.accountsReceivable.filter(a => a.status === 'unpaid').reduce((s,a)=>s+a.amount,0);
        const apTotal = appData.accountsPayable.filter(a => a.status === 'unpaid').reduce((s,a)=>s+a.amount,0);
        ...
        <div class="grid lg:grid-cols-4 gap-4 mb-6">
            ...
            <div class="card-premium p-4"><div class="text-gray-400 text-sm">Waiting Queue</div><div class="text-2xl font-bold text-orange-400">${waitingCount}/15</div></div>
        </div>
        <div class="grid lg:grid-cols-4 gap-4 mb-6">
            <div class="card-premium p-4"><div class="text-gray-400 text-sm">Accounts Receivable</div><div class="text-2xl font-bold text-yellow-400">₱${arTotal.toFixed(2)}</div></div>
            <div class="card-premium p-4"><div class="text-gray-400 text-sm">Accounts Payable</div><div class="text-2xl font-bold text-red-400">₱${apTotal.toFixed(2)}</div></div>
        ...
        <div class="grid lg:grid-cols-4 gap-4 mt-6">
            ...
            <div class="card-premium p-4"><div class="text-gray-400 text-sm">Commissions Paid</div><div class="text-2xl font-bold text-yellow-400">₱${appData.staff.reduce((s,st)=>s+(st.totalCommission||0),0).toFixed(2)}</div></div>
            <div class="card-premium p-4"><div class="text-gray-400 text-sm">Services Active</div><div class="text-2xl font-bold">${appData.services.filter(s=>s.isActive!==false).length}</div></div>
        </div>`;
    }
    ...
    function renderPOS() {
        ...
        return `<div class="grid lg:grid-cols-3 gap-6">
            ...
            <div class="card-premium p-5">
                ...
                <select id="paymentMethod" class="bg-gray-800 p-2 w-full rounded my-2"><option>Cash</option><option>GCash</option><option>Maya</option><option>Credit Card</option><option>Bank Transfer</option></select>
                ...
                <button onclick="addToWaitingListFromPOS()" class="bg-orange-600 w-full p-2 rounded-xl mt-2"><i class="fas fa-clock"></i> Add to Waiting List</button>
                <button onclick="createJobOrderFromPOS()" class="bg-purple-600 w-full p-2 rounded-xl mt-2"><i class="fas fa-clipboard-list"></i> Create Job Order</button>
            </div>
        </div>`;
    }
    ...
    function completeSale() {
        ...
        posCart.forEach(item => {
            const invItem = appData.inventory.find(i => i.name.toLowerCase().includes(item.serviceName.toLowerCase().split(' ')[0]));
            if(invItem) {
    ...
    // ======================== WAITING LIST ========================
    function addToWaitingList(customerName, vehicle, plate, service) {
        if(appData.waitingList.length >= 15) {
            alert("Waiting list is full! (Maximum 15 slots)");
            return null;
        }
        const slot = appData.waitingList.length + 1;
        const entry = {
            id: Date.now(),
            slot: slot,
        ...
        appData.waitingList.push(entry);
        saveToLocal();
        renderApp();
        return entry;
    }

    window.addToWaitingListFromPOS = () => {
        const customer = selectedCustomer ? selectedCustomer.name : prompt("Customer name:") || "Walk-in";
        const vehicle = vehicleInfo.model || prompt("Vehicle model:") || "N/A";
        const plate = vehicleInfo.plate || prompt("Plate number:") || "N/A";
        const service = posCart.length > 0 ? posCart.map(i => i.serviceName).join(', ') : prompt("Service:") || "Pending";
        addToWaitingList(customer, vehicle, plate, service);
    };
    ...
    function renderAdminDashboard() {
        ...
        const adminPanels = [
            ...
            { id: 'ar', icon: 'fa-hand-holding-usd', label: 'Accounts Receivable' },
            { id: 'ap', icon: 'fa-credit-card', label: 'Accounts Payable' },
            { id: 'joborders', icon: 'fa-clipboard-list', label: 'Job Orders' },
        ...
        else if(currentAdminPanel === 'ar') content = renderAccountsReceivable();
        else if(currentAdminPanel === 'ap') content = renderAccountsPayable();
        else if(currentAdminPanel === 'joborders') content = renderJobOrders();
            ...
            <div class="admin-sidebar">
                ...
                <div class="mt-6 p-3 bg-gray-800 rounded-lg text-xs">
                    ...
                    <div class="grid grid-cols-2 gap-1 mt-2">
                        ...
                        <div>Waiting: ${appData.waitingList.filter(w=>w.status!=='completed').length}/15</div>
    ...
    function renderAccountsReceivable() {
        ...
        const totalAR = appData.accountsReceivable.reduce((s,a)=>s+a.amount,0);
        const unpaidAR = appData.accountsReceivable.filter(a => a.status === 'unpaid').reduce((s,a)=>s+a.amount,0);
        return `<div class="card-premium p-6"><h2 class="text-2xl font-bold mb-4">💰 Accounts Receivable</h2>
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div class="bg-gray-800 p-3 rounded"><div class="text-gray-400">Total AR</div><div class="text-xl font-bold">₱${totalAR.toFixed(2)}</div></div>
                <div class="bg-gray-800 p-3 rounded"><div class="text-gray-400">Unpaid</div><div class="text-xl font-bold text-red-400">₱${unpaidAR.toFixed(2)}</div></div>
            </div>
            <button onclick="addAccountsReceivable()" class="bg-green-600 px-3 py-1 rounded mb-4">+ Add AR</button>
            <div class="overflow-x-auto max-h-96 overflow-y-auto"><table class="w-full text-sm"><thead class="sticky top-0 bg-gray-900 border-b border-cyan-400"><tr><th>Customer</th><th>Description</th><th>Amount</th><th>Due Date</th><th>Status</th><th>Actions</th></tr></thead><tbody>
            ${appData.accountsReceivable.map(a => `<tr class="border-b border-gray-700"><td>${a.customer}</td><td>${a.description}</td><td class="text-yellow-400">₱${a.amount}</td><td>${a.dueDate || 'N/A'}</td><td><span class="status-badge status-${a.status}">${a.status}</span></td><td><button onclick="updateARStatus(${a.id})" class="text-cyan-400 mr-2"><i class="fas fa-check"></i></button><button onclick="deleteAR(${a.id})" class="text-red-400"><i class="fas fa-trash"></i></button></td></tr>`).join('')}</tbody></table></div></div>`;
    }

    window.addAccountsReceivable = () => {
        let customer = prompt("Customer name:"); if(!customer) return;
        let amount = parseFloat(prompt("Amount:")); if(isNaN(amount)) return;
        let description = prompt("Description:") || "Service rendered";
        appData.accountsReceivable.push({ id: Date.now(), customer, amount, description, dueDate: "", status: 'unpaid' });
        saveToLocal(); renderApp();
    };
    window.updateARStatus = (id) => {
        let ar = appData.accountsReceivable.find(a => a.id === id);
        if(ar) { ar.status = ar.status === 'unpaid' ? 'paid' : 'unpaid'; saveToLocal(); renderApp(); }
    };
    window.deleteAR = (id) => { if(confirm("Delete AR entry?")) { appData.accountsReceivable = appData.accountsReceivable.filter(a => a.id !== id); saveToLocal(); renderApp(); } };
    ...
    function renderAccountsPayable() {
        ...
        const totalAP = appData.accountsPayable.reduce((s,a)=>s+a.amount,0);
        const unpaidAP = appData.accountsPayable.filter(a => a.status === 'unpaid').reduce((s,a)=>s+a.amount,0);
        return `<div class="card-premium p-6"><h2 class="text-2xl font-bold mb-4">💳 Accounts Payable</h2>
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div class="bg-gray-800 p-3 rounded"><div class="text-gray-400">Total AP</div><div class="text-xl font-bold">₱${totalAP.toFixed(2)}</div></div>
                <div class="bg-gray-800 p-3 rounded"><div class="text-gray-400">Unpaid</div><div class="text-xl font-bold text-red-400">₱${unpaidAP.toFixed(2)}</div></div>
            </div>
            <button onclick="addAccountsPayable()" class="bg-green-600 px-3 py-1 rounded mb-4">+ Add AP</button>
            <div class="overflow-x-auto max-h-96 overflow-y-auto"><table class="w-full text-sm"><thead class="sticky top-0 bg-gray-900 border-b border-cyan-400"><tr><th>Supplier</th><th>Description</th><th>Amount</th><th>Due Date</th><th>Status</th><th>Actions</th></tr></thead><tbody>
            ${appData.accountsPayable.map(a => `<tr class="border-b border-gray-700"><td>${a.supplier}</td><td>${a.description}</td><td class="text-red-400">₱${a.amount}</td><td>${a.dueDate || 'N/A'}</td><td><span class="status-badge status-${a.status}">${a.status}</span></td><td><button onclick="updateAPStatus(${a.id})" class="text-cyan-400 mr-2"><i class="fas fa-check"></i></button><button onclick="deleteAP(${a.id})" class="text-red-400"><i class="fas fa-trash"></i></button></td></tr>`).join('')}</tbody></table></div></div>`;
    }

    window.addAccountsPayable = () => {
        let supplier = prompt("Supplier name:"); if(!supplier) return;
        let amount = parseFloat(prompt("Amount:")); if(isNaN(amount)) return;
        let description = prompt("Description:") || "Purchase";
        appData.accountsPayable.push({ id: Date.now(), supplier, amount, description, dueDate: "", status: 'unpaid' });
        saveToLocal(); renderApp();
    };
    window.updateAPStatus = (id) => {
        let ap = appData.accountsPayable.find(a => a.id === id);
        if(ap) { ap.status = ap.status === 'unpaid' ? 'paid' : 'unpaid'; saveToLocal(); renderApp(); }
    };
    window.deleteAP = (id) => { if(confirm("Delete AP entry?")) { appData.accountsPayable = appData.accountsPayable.filter(a => a.id !== id); saveToLocal(); renderApp(); } };

    // ======================== JOB ORDERS ========================
    window.createJobOrderFromPOS = () => {
        if(posCart.length === 0) { alert("Cart is empty! Add services first."); return; }
        const customer = selectedCustomer ? selectedCustomer.name : prompt("Customer name:") || "Walk-in";
        const vehicle = vehicleInfo.model || prompt("Vehicle model:") || "N/A";
        const plate = vehicleInfo.plate || prompt("Plate number:") || "N/A";
        const jobOrder = {
            id: "JO-" + new Date().getFullYear() + "-" + (appData.nextIds.jobOrder++),
            date: new Date().toISOString(),
            customer: customer,
            vehicle: vehicle,
            plate: plate,
            items: posCart.map(i => ({ service: i.serviceName, variant: i.variant, price: i.price, quantity: i.quantity })),
            total: calcTotal(),
            status: 'pending',
            assignedStaff: posCart.flatMap(i => i.assignedStaff || []).map(s => s.name).filter((v, i, a) => a.indexOf(v) === i),
            remarks: cartRemarks || "No remarks",
            createdAt: new Date().toISOString()
        };
        appData.jobOrders.unshift(jobOrder);
        saveToLocal();
        alert(`✅ Job Order Created!\nID: ${jobOrder.id}\nTotal: ₱${jobOrder.total.toFixed(2)}`);
        renderApp();
    };

    function renderJobOrders() {
        if(showLogin) return renderLogin();
        return `<div class="card-premium p-6"><h2 class="text-2xl font-bold mb-4">📋 Job Orders</h2>
            <div class="overflow-x-auto max-h-96 overflow-y-auto"><table class="w-full text-sm"><thead class="sticky top-0 bg-gray-900 border-b border-cyan-400"><tr><th>ID</th><th>Date</th><th>Customer</th><th>Vehicle</th><th>Total</th><th>Status</th><th>Staff</th><th>Actions</th></tr></thead><tbody>
            ${appData.jobOrders.map(j => `<tr class="border-b border-gray-700"><td class="py-2 font-mono text-xs">${j.id}</td><td>${new Date(j.date).toLocaleDateString()}</td><td>${j.customer}</td><td>${j.vehicle}</td><td class="text-green-400">₱${j.total.toFixed(2)}</td><td><span class="status-badge ${j.status === 'completed' ? 'bg-green-400/20 text-green-400' : 'bg-yellow-400/20 text-yellow-400'}">${j.status}</span></td><td>${j.assignedStaff?.join(', ') || 'Unassigned'}</td><td><button onclick="updateJobStatus('${j.id}')" class="text-cyan-400 mr-2"><i class="fas fa-check"></i></button><button onclick="deleteJobOrder('${j.id}')" class="text-red-400"><i class="fas fa-trash"></i></button></td></tr>`).join('')}</tbody></table></div></div>`;
    }
    window.updateJobStatus = (id) => {
        let job = appData.jobOrders.find(j => j.id === id);
        if(job) { job.status = job.status === 'completed' ? 'pending' : 'completed'; saveToLocal(); renderApp(); }
    };
    window.deleteJobOrder = (id) => { if(confirm("Delete job order?")) { appData.jobOrders = appData.jobOrders.filter(j => j.id !== id); saveToLocal(); renderApp(); } };
    ...
    function renderAdminSystem() {
        ...
        return `<h3 class="text-xl font-bold mb-4"><i class="fas fa-cogs"></i> System Settings</h3>
            ...
            <div class="mt-4 p-3 bg-gray-800 rounded-lg text-xs">
                ...
                <div>Version: 7.0 Enterprise | Last Saved: ${localStorage.getItem('8068_system_last_saved') ? new Date(localStorage.getItem('8068_system_last_saved')).toLocaleString() : 'Never'}</div>
                ...
                <div>Waiting List: ${appData.waitingList.filter(w=>w.status!=='completed').length}/15</div>
                <div>Inventory Items: ${appData.inventory.length}</div>
                <div>AR: ${appData.accountsReceivable.filter(a=>a.status==='unpaid').length} unpaid | AP: ${appData.accountsPayable.filter(a=>a.status==='unpaid').length} unpaid</div>
            </div>`;
    }
        ...
        <div class="max-w-screen-2xl mx-auto px-4 py-5">
            <div class="flex flex-wrap justify-between items-center border-b border-blue-800 pb-4 mb-6">
                <div class="flex items-center gap-3">
                    ...
                    <div><h1 class="text-3xl font-extrabold glow-text">${appData.systemSettings.companyName}</h1><p class="text-sm text-gray-400">${appData.systemSettings.tagline} | Enterprise v7.0</p></div>
                </div>
    ...
    console.log('%c8068 CAR SQUARE - Enterprise Management System v7.0', 'color: #00d4ff; font-size: 20px; font-weight: bold;');
    console.log(`%cRecords: ${appData.transactions.length} TX | ${appData.customers.length} Customers | ${appData.staff.length} Staff | ${appData.inventory.length} Inventory | ${appData.waitingList.filter(w=>w.status!=='completed').length} Waiting`, 'color: #00ff88;');
</script>
</body>
</html>